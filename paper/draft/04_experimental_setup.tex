...
We instantiate the prefill-based methodology (\S\ref{sec:method:extraction}) on one locked $(\text{pool}, \text{dataset}, \text{prompt formatting})$ configuration selected by requirements-first screening (\S\ref{sec:setup:config}).
...
Studies follow a fixed hierarchy: \textbf{signal characterization by dimension} (Studies I--II), \textbf{complementarity across dimensions} (Study III), and \textbf{routing evaluation} (Study IV).
...
We select one primary configuration---a $(\text{pool}, \text{dataset}, \text{prompt formatting})$ triple---using the following criteria:
\begin{enumerate}
...
\textbf{Locked configuration (2026-06-22):} Llama~3.2 1B / 3B Instruct + ARC-Challenge \citep{clark2018think}, deterministic chat template, seed 42.

\input{tables/T1_setup.tex}
...
\textbf{Primary: ARC-Challenge} \citep{clark2018think} --- multiple-choice science reasoning; gold letter match after task formatting (\S\ref{sec:method:extraction}).
...
\textbf{Out-of-distribution generalization:} MMLU \citep{hendrycks2021measuring} transfer is planned future work; this draft reports ARC only (\S\ref{sec:disc:limits}).
...
Calibration selected \texttt{piece\_count} (tokenizer piece count on formatted user content) as $c(q)$ on CALIB ($n{=}299$); all test metrics use this frozen representative.
Full candidate scores are in appendix Table~\ref{tab:d46}.
...
\subsection{Nested evaluation design}
...
\begin{table}[t]
  ...
  \begin{tabular}{lll}
    ...
    \bottomrule
  \end{tabular}
  \caption{Nested calibration and test evaluation.}
  \label{tab:nested}
\end{table}
...
\begin{table}[t]
  ...
  \begin{tabular}{llll}
    ...
    \midrule
    I  & Signal characterization & Which dimension predicts opportunity? (RH1) &
      $\rho$, AUROC, AUPRC, bucket distributions \\
    II & Signal characterization & Do dimensions encode distinct routing need? (RH1--RH2) &
      Same (+ alternate statistics in appendix) \\
    III & Complementarity & Complementary information across dimensions (RH3) &
      $\Delta$AUROC: task difficulty $\rightarrow$ + model uncertainty $\rightarrow$ joint; DeLong \\
    IV & Routing evaluation & Can a calibrated policy exploit available information? (RH4) &
