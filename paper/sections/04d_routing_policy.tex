...
Higher $s(q)$ indicates stronger evidence that $\Mhi$ is the \textbf{appropriate choice} ($r(q)=1$ under the oracle); $\pi(q)$ \textbf{selects} $\Mhi$ when $s(q) > \tau$, otherwise \textbf{selects} $\Mlo$ (\textbf{binary model selection}).
